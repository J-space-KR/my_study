    ...
    /* script.js 파일의 기본일기목록을 아래 내용으로 교체하세요 */
const 기본일기목록 = [
    { 번호: 1786668363568, 감정: "행복해요", 이모지: "😊", 날짜: "2026. 08. 03", 제목: "새싹 첫 날!", 내용: "." },
    { 번호: 1786668604781, 감정: "행복해요", 이모지: "😊", 날짜: "2026. 08. 04", 제목: "라이브쉐어가 도입되었다. 너무 도움이 된다. 이거 제안한 분, 상 줘야한다!", 내용: "." },
    { 번호: 1786668752406, 감정: "기타", 이모지: "🤔", 날짜: "2026. 08. 05", 제목: "그리드에 대해서 배움.", 내용: "." },
    { 번호: 1786668838003, 감정: "슬퍼요", 이모지: "😢", 날짜: "2026. 08. 06", 제목: "라이브쉐어가 또 먹통", 내용: "." },
    { 번호: 1786668900684, 감정: "기타", 이모지: "🤔", 날짜: "2026. 08. 07", 제목: "잠 부족해서 엄청 피곤한 상태", 내용: "." },
    { 번호: 1786669005051, 감정: "놀랐어요", 이모지: "😲", 날짜: "2026. 08. 08", 제목: "멀티탭 먹통인 것 발견", 내용: "." },
    { 번호: 1786694717439, 감정: "행복해요", 이모지: "😊", 날짜: "2026. 08. 09", 제목: "test", 내용: ".테스트 테스트" },
    { 번호: 1786694822402, 감정: "화나요", 이모지: "😠", 날짜: "2026. 08. 10", 제목: "어려워서 소리지르면서 뛰쳐나갈 뻔", 내용: "." }
]
        ...
        function 감정이미지가져오기(감정) {
    if (감정 === "행복해요") return "./assets/행복.png"
    if (감정 === "슬퍼요") return "./assets/슬픔.png"
    if (감정 === "놀랐어요") return "./assets/놀람.png"
    if (감정 === "화나요") return "./assets/화남.png"
    return "./assets/기타.png"
}
            ...
            return `
                <div class="카드" onclick="상세페이지가기(${일기.번호})">
                    <div class="사진상자 ${감정배경가져오기(일기.감정)}">
                      <div class="이모지원">
    <img class="이모지이미지" src="${감정이미지가져오기(일기.감정)}" alt="${일기.감정}" />
</div>
        ...
        /* 한 페이지에 일기 8개만 잘라서 보여줘요. */
        function 일기목록그리기(보여줄목록, 클릭한페이지) {
            if (클릭한페이지 === undefined) 클릭한페이지 = 1

            페이지에보여줄일기목록 = 보여줄목록

            const 시작번호 = (클릭한페이지 - 1) * 8
            const 현재페이지목록 = 보여줄목록.slice(시작번호, 시작번호 + 8)
            let 카드HTML = ""

            현재페이지목록.map(function (일기) {
            ...
            페이지버튼그리기(보여줄목록.length, 클릭한페이지)
        }

        function 페이지버튼그리기(전체개수, 클릭한페이지) {
            const 마지막페이지 = Math.ceil(전체개수 / 8)

            const 페이지HTML = new Array(마지막페이지).fill("페이지").map(function (페이지, 순서) {
                const 페이지번호 = 순서 + 1

                return `
                    <button
                        class="페이지번호 ${클릭한페이지 === 페이지번호 ? "활성페이지" : ""}"
                        onclick="일기목록그리기(페이지에보여줄일기목록, ${페이지번호})"
                    >${페이지번호}</button>
                `
            }).join("")

            const 이전버튼 = `
                <button class="페이지번호" onclick="일기목록그리기(페이지에보여줄일기목록, ${클릭한페이지 - 1})" ${클릭한페이지 === 1 ? "disabled" : ""}>&lt;</button>
            `
            const 다음버튼 = `
                <button class="페이지번호" onclick="일기목록그리기(페이지에보여줄일기목록, ${클릭한페이지 + 1})" ${클릭한페이지 >= 마지막페이지 ? "disabled" : ""}>&gt;</button>
            `

            document.getElementById("페이지번호영역").innerHTML = 이전버튼 + 페이지HTML + 다음버튼
        }
        ...
        function 강아지사진불러오기() {
            if (사진불러오는중 === true) return

            const 사진목록 = document.getElementById("강아지사진목록")
            const 처음불러오는지 = 강아지사진을불러왔는지 === false
            ...
            fetch("https://dog.ceo/api/breeds/image/random/10")
                ...
                .then(function (객체만뽑힌결과) {
                    const 사진HTML = 객체만뽑힌결과.message.map(function (사진주소) {
                        return 강아지사진컴포넌트(사진주소)
                    }).join("")

                    if (처음불러오는지 === true) {
                        사진목록.innerHTML = 사진HTML
                    } else {
                        사진목록.innerHTML += 사진HTML
                    }
        ...
        function 감정바꾸기(event) {
            ...
            document.getElementById("필터드롭다운").style = `--선택한필터: "${선택한감정}"`
            document.getElementById("필터드롭다운").click()
            일기검색하기()
        }
        ...
        function 메인다크모드바꾸기() {
            document.body.classList.toggle("메인다크모드")
        }
    ...
    내용: document.getElementById("내용").value
        ...
        /* 사진 탭에서 70%까지 내리면 10장을 더 불러와요. */
        window.addEventListener("scroll", function () {
            if (스크롤타이머 !== "아직실행안함") return

            스크롤타이머 = setTimeout(function () {
                스크롤타이머 = "아직실행안함"
            }, 700)

            if (document.getElementById("사진메뉴").style.display !== "block") return

            const 스크롤퍼센트 = document.documentElement.scrollTop /
